(() => {
  ...
  const PAGES = [
    ["task-board", "Care Task Board"], ["todays-routine", "Today's Schedule"], ["chat-bot-dbt-skills", "DBT Support"], ["dbt-daily-cards", "DBT Daily Cards"], ["dbt-journaling", "DBT Journaling"], ["mobile-games", "Mobile Games"], ["serotonin", "Serotonin"], ["squishy-store", "Squishy Store"]
  ];
  ...
  const TASKS = [
    ...
    t("use-chatbot","Use the support chat for a check-in","DBT","High","daily","Support counts before things become a crisis.",1600,160,40,4),
    t("use-chatbot-distress","Use the support chat during distress","DBT","Very High","asneeded","Reaching for support during distress is huge.",3600,360,120,16),
  ...
  const SUPPORT_MOODS = {
    caring:{label:"Care support",note:"A steady, practical care response focused on safety and tiny next steps."},
    listening:{label:"Listening mode",note:"Listening first, then choosing a skill only when it is useful."},
    snuggly:{label:"Comfort mode",note:"Gentle reassurance, body-softening, and lower-demand support."},
    purring:{label:"Grounding mode",note:"Slow-breath, sensory, and nervous-system settling support."},
    grounding:{label:"Grounding mode",note:"Slow-breath, sensory, and nervous-system settling support."},
    thinking:{label:"Thinking mode",note:"Sorting the problem into facts, feelings, needs, and one small action."},
    thoughtful:{label:"Thoughtful support",note:"Kind-truth support without shame or punishment."},
    advising_professor:{label:"Planning mode",note:"Skill selection, tiny steps, reminders, and practical planning."},
    advising:{label:"Planning mode",note:"Skill selection, tiny steps, reminders, and practical planning."},
    judgmental:{label:"Care-check mode",note:"Used only to redirect skipped body care back toward food, meds-as-prescribed, hygiene, hydration, or rest."},
...
function renderGameSessionStatus(){
  ...
  function renderChat(){
    const p=personaData();
    const initial=random(p.greetings||["Hi Jasper. The support chat is ready for DBT, ADHD tiny steps, grounding, self-care resets, diary cards, and reward check-ins."]);
...
Because this looks heavier than a normal check-in, I am going to keep the next step small, concrete, and body-safe.`;
  ...
  function messageHtml(m){
    const moodTag=m.mood?`<span class="mood-tag">${escapeHtml((SUPPORT_MOODS[m.mood]||{}).label||m.mood)}</span>`:"";
    return `<div class="message ${m.role}"><span class="message-speaker">${m.role==="bot"?"Support":"You"}${m.role==="bot"?moodTag:""}</span>${escapeHtml(m.text)}</div>`;
  }
  ...
  function supportData(){ return (window.JASPER_SUPPORT_DATA||{}).supportSkills || (window.JASPER_SUPPORT_DATA||{}); }
  function personaData(){ return {
    greetings:["Hi Jasper. The support chat is ready. Tell me what is happening, or ask for DBT, TIPP, ADHD tiny steps, self-care reset, attachment-alarm support, or grounding."],
    careLines:["Care comes before shame.","Start with the smallest safe next step.","Your feelings are information, not a verdict."],
    dbtLines:["Skill practice counts even when it is messy.","Pick the skill that lowers danger or demand first.","DBT is a tool, not a punishment."],
    adhdLines:["Starting gets rewarded before finishing.","Make the task smaller until it stops hissing.","One tiny step is still task movement."],
    crisisLines:["This sounds bigger than cozy chat, and you deserve live human help now."],
    idleLines:["Quiet counts. Rest counts. Pausing before reacting counts."],
    comfortLines:["You do not have to solve everything at once.","Let the next step be smaller and kinder.","A hard-day version of care still counts."],
    pepTalkLines:["Do the next gentle thing. Not the perfect thing. The next gentle thing."],
    signoffs:["Tiny wins are real wins.","Care first, shame never.","One step counts."]
  }; }
  ...
  function canonicalMoodKey(key){
    ...
    return SUPPORT_MOODS[key]?key:"caring";
  }
  function moodLine(moodKey, fallback=""){
    const p=personaData();
    const mood=canonicalMoodKey(moodKey);
    if(mood==="listening") return pick(["I am listening first.","You can put one piece down at a time."], fallback||"I am listening.");
    if(mood==="purring") return pick(["Let’s ground the body first.","Slow breath first, problem-solving second."], fallback||"Grounding first.");
    if(mood==="advising_professor") return pick(p.dbtLines, fallback||"Tiny plan first.");
    return pick(p.careLines, fallback||"Care comes before shame.");
  }
  ...
  function buildWho(){
    ...
    return `This is Jasper's DBT + ADHD support chat with passive scanner support. The full Emperor Onyx companion page is linked from the chat module and remains available as a separate secondary system.\n\nDBT helper skills include: ${dbtTitles||"STOP, TIPP, Wise Mind, Check the Facts, Opposite Action, PLEASE, DEAR MAN/GIVE/FAST"}.\n\nADHD helper tools include: ${adhdTitles||"tiny task splitter, body doubling, reminders, transitions, and shame-free resets"}.\n\nIt can coach skills, grounding, diary cards, routines, and safety nudges, but it is not licensed therapy, medical care, or emergency care.`;
  }
  ...
  function supportAnswer(text, scanReport=null){
    ...
    const hasIntent=name=>includesAny(q,(supportData().chatIntents||{})[name]||[]);
    ...
    if(q.includes("hungry")||q.includes("food")) return {mood:"caring",text:"Food check: please get yourself a tiny safe food or drink if your body needs one. Eating counts even when it is simple."};
  ...
  function setSupportMood(key){
    ...
    const mood=SUPPORT_MOODS[canonical]||SUPPORT_MOODS.caring;
    const label=$("#supportMoodLabel"), note=$("#supportMoodNote");
    if(label) label.textContent=mood.label||canonical;
    if(note) note.textContent=mood.note||"Support is ready.";
  ...
  function setFloatingButtonMood(key){
    ...
    if(btn) btn.setAttribute("aria-label", `Open quick support: ${mood.label||canonical}`);
  }
  ...
  function renderSupportResources(){
    ...
    el.innerHTML=`<article><h3>Support resources kept</h3><p>This module keeps DBT, ADHD, attachment-alarm, diary-card, rewards, mobile-game support, and passive background scanner context. Emperor Onyx remains linked as the full companion page.</p></article><article><h4>DBT skills</h4><p class="lore-chip-row">${chips(dbt,24)}</p></article><article><h4>ADHD tools</h4><p class="lore-chip-row">${chips(adhd,18)}</p></article><article><h4>Safety note</h4><p>${escapeHtml(support.clinicalSafety?.shortDisclaimer||"This can coach skills, grounding, routines, and self-care check-ins, but it is not licensed therapy, medical care, or emergency care.")}</p></article>`;
  }
