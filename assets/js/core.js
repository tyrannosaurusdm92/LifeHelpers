...
(() => {
  ...
  const DATA_PROFILE = String(window.OURSPACE_DATA_PROFILE || config.dataProfile || PROFILE).toLowerCase();
  const JSON_PROFILE = (DATA_PROFILE.includes("jasper") || DATA_PROFILE.includes("squishy") || DATA_PROFILE.includes("momma")) ? "jasper" : (DATA_PROFILE.includes("william") || DATA_PROFILE.includes("dino") || DATA_PROFILE.includes("papa")) ? "william" : DATA_PROFILE;
  ...
  const dataPaths = {
    tasks: `json/${JSON_PROFILE}/${JSON_PROFILE}_tasks.json`,
    schedule: `json/${JSON_PROFILE}/${JSON_PROFILE}_schedule.json`,
    calendar: `json/${JSON_PROFILE}/${JSON_PROFILE}_calendar.json`,
    store: `json/${JSON_PROFILE}/${JSON_PROFILE}_store.json`,
    hobbies: `json/${JSON_PROFILE}/${JSON_PROFILE}_hobby.json`,
    gamesManifest: "json/games/games_manifest_merged.json",
    allowedGameIds: "json/games/allowed_game_ids_merged.json",
    gameRewardRules: "json/games/game_reward_rules_merged.json",
    dbtCombined: "json/dbt/dbt_combined_catalog.json",
    dbtSkills: "json/dbt/dbt_skills_catalog.json",
