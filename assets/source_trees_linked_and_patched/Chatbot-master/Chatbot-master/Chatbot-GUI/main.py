...
MERGED_SYSTEM_ROOT = Path(__file__).resolve().parents[4]
...
def getresponse(text, lang_code, sech):
    ...
    text_input = dialogflow.types.TextInput(language_code=lang_code, text=augmented_text)
    ...
    if not response.query_result.fulfillment_text:
        ...
        response = background_psychiatry_scanner.enrich_response(response, background_report)
    return response
