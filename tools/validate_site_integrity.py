...
def node_syntax_check():
    files=sorted((ROOT/'js').glob('*.js')) + [ROOT/'games/developer_v1.js']
    out=[]
    for f in files:
...
def app_integrity_check():
    idx=ROOT/'index.html'; onyx=ROOT/'Emperor_onyx.html'; app=ROOT/'js/app.js'
    index=idx.read_text(errors='ignore'); appjs=app.read_text(errors='ignore'); onyxhtml=onyx.read_text(errors='ignore')
    ...
    return {
        ...
        'has_typeform_link': 'https://form.typeform.com/to/trAqvrRG' in index or 'https://form.typeform.com/to/trAqvrRG' in onyxhtml,
        ...
        'has_onyx_secondary_link': 'href="Emperor_onyx.html"' in index,
        'onyx_scanner_loaded': 'unified-background-psychiatry-runtime.js' in onyxhtml,
...
def main():
    ...
    required=['index_title_ok','scanner_scripts_before_app','has_typeform_link','has_checkout_email','checkout_deducts_before_mailto','has_onyx_secondary_link','onyx_scanner_loaded','currency_conversion_10s','legacy_tabletennis_stub_exists','passive_scanner_status_element','scanner_report_stored']
    for key in required:
