...
(function attachConversationScannerRandomizer(root) {
  ...
  const DEFAULT_RULES = {
    ...
    answerKeys: ["answer", "answers", "response", "responses", "reply", "replies", "message", "messages", "text", "content", "body", "line", "lines", "template", "templates", "variant", "variants", "phrase", "phrases", "output", "fallback"],
  ...
  async function loadJsonFiles(files) {
    ...
    for (let i = 0; i < input.length; i += 1) {
      ...
      if (name.indexOf("/json/") === -1 && name.slice(0, 5).toLowerCase() !== "json/" && name.indexOf("json/") !== 0) {
        if (input.length > 1 && name.indexOf("/") !== -1) continue;
      }
      try {
  ...
  function collectIntentEntries(container, fileName, route, meta, out) {
    ...
    for (let i = 0; i < keys.length; i += 1) {
      ...
      const answerFields = [obj.templates, obj.responses, obj.replies, obj.messages, obj.lines, obj.phrases, obj.fallback];
  ...
  function selectCandidate(top, inputText, options) {
    ...
    for (let i = 0; i < usable.length; i += 1) {
      if (!recent[cleanText(usable[i].entry.text)]) fresh.push(usable[i]);
    }
